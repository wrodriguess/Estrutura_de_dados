...
void opcao(node *FILA, int op){
 ...
 switch(op){
  ...
  case 1:
   contaAvioes(FILA); //TENHO QUE FAZER
  break;

  case 2:
  tmp= retira(FILA);
   if(tmp != NULL){
    printf("Retirado: %3d\n\n", tmp->avioes->identificador);
    libera(tmp);
   }
  break;
...
node *aloca(){
 ...
 if(!novo){
  ...
  printf("Novo elemento: "); scanf("%d", &novo->avioes->identificador);
  return novo;
 }
}
...
void exibe(node *FILA){
 ...
 printf("Fila :");
 while( tmp != NULL){
  printf("%5d", tmp->avioes->identificador);
  tmp = tmp->prox;
 }
 printf("\n        ");
 int count;
